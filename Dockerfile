...
# Create memory detection and optimization script
RUN cat > /tmp/memory-check.sh << 'EOF'
#!/bin/bash
...
# Export for use in npm command
echo "export NODE_OPTIONS=\"$NODE_OPTIONS\"" > /tmp/node-env
echo "export NPM_FLAGS=\"$NPM_FLAGS\"" >> /tmp/node-env
EOF
...
RUN echo "=== Build Stage Memory Check ===" && \
    /tmp/memory-check.sh && \
    source /tmp/node-env && \
